...
const props = defineProps({
  load: {
    required: true,
    type: Function as PropType<(action: LoadAction, payload: LoadPayload) => void>,
  },
  isInitial: {
    required: false,
    type: Boolean,
...
function getScrollHeightFromEl($el: HTMLElement): number {
  return $el.scrollHeight;
}

function getScrollWidthFromEl($el: HTMLElement): number {
  return $el.scrollWidth;
}

function restoreScrollVerticalPosition($el: HTMLElement, scrollHeight: number): void {
  $el.scrollTop = $el.scrollHeight - scrollHeight + $el.scrollTop;
}

function restoreScrollHorizontalPosition($el: HTMLElement, scrollWidth: number): void {
...
const isFirstLoad = ref(props.isInitial);
  ...
  return new IntersectionObserver(
    ([entry]) => {
      if (entry.isIntersecting) {
        if (props.position === 'top') {
          scrollSize = getScrollHeightFromEl(props.container ?? document.documentElement);
        } else if (props.position === 'left') {
          scrollSize = getScrollWidthFromEl(props.container ?? document.documentElement);
        }
        unobserve();
        props.load(
          {
...
<template>
  <div class="vue-eternal-loading" ref="rootRef">
    ...
    <slot v-else-if="state === 'no-results'" v-bind="{ retry }" name="no-results">
