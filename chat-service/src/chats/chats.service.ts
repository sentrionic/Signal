...
import { Services, Subjects, ChatMembersAddedEvent } from '@senorg/common';
...
export class ChatsService {
  ...
  async getOrCreateChat(currentID: string, contactID: string): Promise<ChatResponse> {
    const chat = await this.chatRepository.findOne(
      {
        $and: [{ members: { user: { id: currentID } } }, { members: { user: { id: contactID } } }],
      },
    ...
    this.notificationClient.emit(
      ...
      new ChatMembersAddedEvent([current.id, contact.id], newChat.id),
    );
